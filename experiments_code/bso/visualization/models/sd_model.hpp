...
public:
	...
	const std::string get_description();
        ...
        bso::utilities::geometry::vertex calculateCenter(const std::vector<bso::utilities::geometry::vertex>& vertices) {
        bso::utilities::geometry::vertex center({1.0, 0.0, 0.0});
...
SD_Model::SD_Model(const bso::structural_design::sd_model& sd,
									 ...
									 const std::vector<std::pair<bso::utilities::geometry::vertex,
									 bso::utilities::geometry::vector>>& cuttingPlanes /*= {}*/)
: model_base(cuttingPlanes), label_counter(1)
{
	mTitle = title;
	...
	if (type == "component")
	{
		for (const auto& i : sd.getGeometries())
		{
			for (const auto& j : i->getStructures())
			{
                ...
                labelStream << "Rect " << label_counter++; // Create label text with increment

                // Calculate position using the helper function
                std::vector<bso::utilities::geometry::vertex> flatVertices;
                for (const auto& pointsVector : i->getElementPoints()) {
                    for (const auto point : pointsVector) {
                        // Assuming that point* points to an object that can be implicitly converted to vertex
                        flatVertices.push_back(*point);  // Dereference the point to convert or access the vertex
                    }
                }

                bso::utilities::geometry::vertex labelPos = calculateCenter(flatVertices);

                // Create and add the label
                std::string labelText = labelStream.str();  // Convert stringstream to string
				...
				else if (j.type() == "flat_shell")
				{
					...
					if (j.isVisible())
					{
						...
						//this->addLabel(labels, lbprops, labelText, labelPos);
					}
					else if (ghostly)
					{
