#ifndef BSO_GRAMMAR_DESIGN_TANGRAM_CPP
#define BSO_GRAMMAR_DESIGN_TANGRAM_CPP

#include <bso/grammar/grammar.hpp>  // Make sure this path correctly points to your grammar.hpp file
#include <bso/utilities/clustering.hpp>
#include <stdexcept>
#include <sstream>

namespace bso { namespace grammar {
class DESIGN_TANGRAM;

template <>
bso::structural_design::sd_model grammar::sd_grammar<DESIGN_TANGRAM>(
    ...
    const bso::structural_design::component::structure& substituteStructure)
{
	...
	for (int i = 0; i <= etaConverge + 1; ++i) {
		...
		{
			doClustering = true;
			bso::utilities::clustering subClustering(resultData,kMin,kMax,10);
...
#endif // BSO_GRAMMAR_DESIGN_TANGRAM_CPP
